...
def get_media(media_type: str):
    ...
    if sort_by not in (None, 'length', 'random', 'releasedate', 'criticsrating', 'myrating'):
...
def put_media(media_type: str):
    ...
    media_type: MEDIA_TYPE
    imdb = 'noimdb' not in request.form
    return upsert_media(media_type='movie', insert=True, replace=True, imdb_fetch=imdb)
...
def patch_media(media_type: str):
    ...
    return upsert_media(media_type='movie', insert=True, replace=True, imdb_fetch=False)
